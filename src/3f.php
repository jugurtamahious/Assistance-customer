...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
        ...
        if(isset($_POST['creer']))
  {
      if(! isset($privilegecy['usertype'])){
        ...
        if(! in_array($extension ,['zip','pdf','png','jpg','svg'])){
          
          echo 'vous devez rentrer un ficher de type .zip , .pdf , .png , .jpg , .svg ';
            ...
            if(move_uploaded_file($file, $destination)){
                ...
                if(!empty($travaux) && !empty($demande) && !empty($date) &&  !empty($file) && isset($_SESSION['id']))
                {
                    ...
                    try {
                        ...
                        $pdo = new PDO("mysql:host=database:3306;dbname=php_db", "root", "password");
                          $queryy = $pdo->prepare("INSERT INTO 3f (travaux, demande,eventdate, filesname,size, user_id) VALUES (:travaux, :demande,:eventdate, :filesname,:size, :user_id)");
                        $queryy->execute([
                          ":travaux" => $travaux,
                          ":demande" => $demande,
                          ":eventdate" => $date,
...
<div class="field is-grouped">
  <div class="control">
    <button type="submit" class="button is-primary" name="filtrer"><a href="filtrer.php?3f">filtrer</a></button>
  </div>
</div>
<div class="field is-grouped">
  <div class="control">
...
if(isset($privilegecy['usertype'])){
  $query = $pdo->prepare("SELECT * FROM user inner join 3f on user.id = 3f.user_id");
  $query->execute();
...
elseif(!  isset($privilegecy['usertype'])){
  userForm();
  $query = $pdo->prepare("SELECT * FROM user inner join 3f on user.id = 3f.user_id WHERE `user`.`id` LIKE {$_SESSION['id']}");
  $query->execute();
      ...
      ?>
               ...
               <div class = "container_table">
                    <table class="table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
                      <thead>
                        <tr>
                        <th><abbr title="Numero de la demande">Numero de la demade</abbr></th>
     ...
     
       {
                      ...
                      <tr>
                         <th>{$onepost['id']}</th>
                          ...
                          <td>
                          {$onepost['filesname']}<a href='filelogic.php?3ffilesid={$onepost['id']}'>Télecharger</a></td>
                          ...
                          if(isset($privilegecy['usertype'])){
                            ...
                            echo"
                            <td><button type='submit' class='button is-primary' name='response'><a href='response.php?3fresponseid={$onepost['id']}'>répondre</a></button>
                            <button type='submit' class='button is-primary' name='supprimer'><a href='delete.php?3fdeleteid={$onepost['id']}'>supprimer</a>
                            </button></td>
                           
                            ";
                          }elseif(!isset($privilegecy['usertype'])){
                            echo" 
                            <td><button type='submit' class='button is-primary' name='supprimer'><a href='delete.php?3fdeleteid={$onepost['id']}'>supprimer</a>
                            </button></td>
                          ...
                          echo"
                             <td>
                                 <button class='like__btn'>
                                   <a href='likeslogic.php?3flike={$onepost['id']}'>
                                   <span id='icon'><i class'far fa-thumbs-up'></i></span>
                                   <span id='count'>{$onepost['likes']}</span> Like
                                   </a>
                                </button>
                                <button class='like__btn'>
                                  <a href='likeslogic.php?3fdislike={$onepost['id']}'>
                                   <span id='icon'><i class'far fa-thumbs-up'></i></span>
                                   <span id='count'>{$onepost['dislikes']}</span> Disike
                                    </a>
                                </button>
                            </td>
                          ";
