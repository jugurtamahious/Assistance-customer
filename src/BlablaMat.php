      ...
      if(move_uploaded_file($file, $destination)){
          ...
          if(!empty($travaux) && !empty($demande) && !empty($date) &&  !empty($file) && isset($_SESSION['id']))
          {
              ...
              try {
                  ...
                  $pdo = new PDO("mysql:host=database:3306;dbname=php_db", "root", "password");
                  $queryy = $pdo->prepare("INSERT INTO post (travaux, demande,eventdate, filesname,size, user_id) VALUES (:travaux, :demande,:eventdate, :filesname,:size, :user_id)");
                  $queryy->execute([
                    ":travaux" => $travaux,
                    ":demande" => $demande,
                    ":eventdate" => $date,
...
<div class="field is-grouped">
  <div class="control">
    <button type="submit" class="button is-primary" name="filtrer"><a href="filtrer.php?Blablamat">filtrer</a></button>
  </div>
</div>
<div class="field is-grouped">
  <div class="control">
...
if(isset($privilegecy['usertype'])){
  $query = $pdo->prepare("SELECT * FROM user inner join post on user.id = post.user_id");
  $query->execute();
  $allpost = $query->fetchAll(PDO::FETCH_ASSOC);
}
elseif(! isset($privilegecy['usertype'])){
  userForm();
  $query = $pdo->prepare("SELECT * FROM user inner join post on user.id = post.user_id  WHERE `user`.`id` LIKE {$_SESSION['id']}");
  $query->execute();
  $allpost = $query->fetchAll(PDO::FETCH_ASSOC);
}
   ?> 
     ...
     
       {
                      ...
                      <tr>
                          ...
                          <td>{$onepost['filesname']}<a href='filelogic.php?filesid={$onepost['id']}'>Télecharger</a></td>
                          ";
                          if(isset($privilegecy['usertype'])){
                            ...
                            echo"
                              <td>
                                 <button class='like__btn'>
                                   <a href='likeslogic.php?like={$onepost['id']}'>
                                   <span id='icon'><i class'far fa-thumbs-up'></i></span>
                                   <span id='count'>{$onepost['likes']}</span> Like
                                   </a>
                                </button>
                                <button class='like__btn'>
                                  <a href='likeslogic.php?dislike={$onepost['id']}'>
                                   <span id='icon'><i class'far fa-thumbs-up'></i></span>
