      ...
      if(move_uploaded_file($file, $destination)){
          ...
          if(!empty($travaux) && !empty($demande) && !empty($date) &&  !empty($file) && isset($_SESSION['id']))
          {
              ...
              try {
                  ...
                  $pdo = new PDO("mysql:host=database:3306;dbname=php_db", "root", "password");
                  $queryy = $pdo->prepare("INSERT INTO challenges (travaux, demande,eventdate, filesname,size, user_id) VALUES (:travaux, :demande,:eventdate, :filesname,:size, :user_id)");
                  $queryy->execute([
                      ":travaux" => $travaux,
                      ":demande" => $demande,
                      ":eventdate" => $date,
...
<div class="field is-grouped">
  <div class="control">
    <button type="submit" class="button is-primary" name="filtrer"><a href="filtrer.php?challenges">filtrer</a></button>
  </div>
</div>
<div class="field is-grouped">
  <div class="control">
...
if(isset($privilegecy['usertype'])){
  $query = $pdo->prepare("SELECT * FROM user inner join challenges on user.id = challenges.user_id ");
  $query->execute();
...
elseif(!  isset($privilegecy['usertype'])){
  userForm();
  $query = $pdo->prepare("SELECT * FROM user inner join challenges on user.id = challenges.user_id WHERE `user`.`id` LIKE {$_SESSION['id']}");
  $query->execute();
  $allpost = $query->fetchAll(PDO::FETCH_ASSOC);
  echo'utilisateur';                                    
}
      ...
      ?>
     ...
     
       {
                      ...
                      <tr>
                          ...
                          <td>{$onepost['filesname']}<a href='post.php?challengesfilesid={$onepost['id']}'>Télecharger</a></td>
                          ";
                          if(isset($privilegecy['usertype'])){
                         ...
                         <td>{$onepost['timespent']}</td>
                            ...
                            <td><button type='submit' class='button is-primary' name='response'><a href='response.php?challengesresponseid={$onepost['id']}'>répondre</a></button>
                            <button type='submit' class='button is-primary' name='supprimer'><a href='delete.php?challengesdeleteid={$onepost['id']}'>supprimer</a>
                            </button></td>
                            ";
                          }elseif(!isset($privilegecy['usertype'])){
                            echo" 
                            ...
                            <td>{$onepost['timespent']}</td>
                            <td>{$onepost['response']}</td>
                            <td><button type='submit' class='button is-primary' name='supprimer'><a href='delete.php?challengesdeleteid={$onepost['id']}'>supprimer</a>
                            </button></td>
                            ...
                            echo"
                              <td>
                                 <button class='like__btn'>
                                   <a href='likeslogic.php?challengeslike={$onepost['id']}'>
                                   <span id='icon'><i class'far fa-thumbs-up'></i></span>
                                   <span id='count'>{$onepost['likes']}</span> Like
                                   </a>
                                </button>
                                <button class='like__btn'>
                                  <a href='likeslogic.php?challengesdislike={$onepost['id']}'>
                                   <span id='icon'><i class'far fa-thumbs-up'></i></span>
