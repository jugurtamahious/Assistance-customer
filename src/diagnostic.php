      ...
      if(move_uploaded_file($file, $destination)){
          ...
          if(!empty($travaux) && !empty($demande) && !empty($date) &&  !empty($file) && isset($_SESSION['id']))
          {
              ...
              try {
                  ...
                  $pdo = new PDO("mysql:host=database:3306;dbname=php_db", "root", "password");
                  $queryy = $pdo->prepare("INSERT INTO diagnostic (travaux, demande,eventdate, filesname,size, user_id) VALUES (:travaux, :demande,:eventdate, :filesname,:size, :user_id)");
                  $queryy->execute([
                    ":travaux" => $travaux,
                    ":demande" => $demande,
                    ":eventdate" => $date,
...
<div class="field is-grouped">
  <div class="control">
    <button type="submit" class="button is-primary" name="filtrer?diagnostic"><a href="filtrer.php">filtrer</a></button>
  </div>
</div>
<div class="field is-grouped">
  <div class="control">
...
$query = $pdo->prepare("SELECT * FROM user inner join diagnostic on user.id = diagnostic.user_id");
  $query->execute();
      ...
      ?>
     ...
     
       {
                      ...
                      <tr>
                          ...
                          <td>
                          {$onepost['filesname']}<a href='filelogic.php?diagnosticfilesid={$onepost['id']}'>Télecharger</a></td>
                          ";
                          if(isset($privilegecy['usertype'])){
                         ...
                         <td>{$onepost['timespent']}</td>
                            ...
                            <td><button type='submit' class='button is-primary' name='response'><a href='response.php?diagnosticresponseid={$onepost['id']}'>répondre</a></button>
                            <button type='submit' class='button is-primary' name='supprimer'><a href='delete.php?diagnosticdeleteid={$onepost['id']}'>supprimer</a>
                            </button></td>
                            ";
                          }elseif(!isset($privilegecy['usertype'])){
                            echo" 
                            ...
                            <td>{$onepost['timespent']}</td>
                            <td>{$onepost['response']}</td>
                            <td><button type='submit' class='button is-primary' name='supprimer'><a href='delete.php?diagnosticdeleteid={$onepost['id']}'>supprimer</a>
                            </button></td>
                        ...
                        ";}
                          ...
                          <td>
                                 <button class='like__btn'>
                                   <a href='likeslogic.php?diagnosticlike={$onepost['id']}'>
                                   <span id='icon'><i class'far fa-thumbs-up'></i></span>
                                   <span id='count'>{$onepost['likes']}</span> Like
                                   </a>
                                </button>
                                <button class='like__btn'>
                                  <a href='likeslogic.php?diagnosticdislike={$onepost['id']}'>
                                   <span id='icon'><i class'far fa-thumbs-up'></i></span>
